...
export class Pool {
  ...
  release(slot, outcome) {
    ...
    switch (outcome) {
      case 'ok':
        slot.stats.ok++;
        slot.consecDepleted = 0;
        break;
      case 'depleted':
        slot.stats.depleted++;
        slot.cooldownUntil = now() + config.cooldownMs;
        // Keep the cookie: the wall is per-IP (a fresh cookie on the same IP is
        // walled too), so re-harvesting buys nothing and costs ~2.6s. The same
        // A/lid mint a fresh chat_token fine once the IP recovers. But if the lid
        // itself died we'd loop depleted forever, so after several straight
        // depletions force a fresh harvest.
        if (++slot.consecDepleted >= 3) { slot.cookie = null; slot.consecDepleted = 0; }
        break;
      case 'error':
      default:
        slot.stats.errors++;
  ...
  async warmOnce() {
    if (this._warming) return; // never let interval ticks stack on a slow harvest
    this._warming = true;
    try {
      // harvest all cold slots concurrently (boot warm ~one harvest, not N)
      await Promise.all(this.slots.map(async (slot) => {
        if (slot.busy || slot.cooling() || !slot.cookieStale()) return;
        slot.busy = true;
  ...
  startWarmer(intervalMs = 3000) {
    this.warmOnce(); // harvest immediately at boot so the first request is warm
    this._warmer = setInterval(() => { this.warmOnce(); }, intervalMs);
