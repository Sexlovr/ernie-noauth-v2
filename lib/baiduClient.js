...
export function buildBody(query, session, model) {
  ...
  // modelFunction carries the per-model toggles (internetSearch/deepSearch/thinkMode);
  // resolveModel() builds it. Fall back to a bare smartMode shape if absent.
  const modelFunction = model.modelFunction || { internetSearch: '0', deepSearch: '0', thinkMode: '0' };
  const body = {
    message: {
      ...
      searchInfo: {
        ...
        isPrivateChat: false,
        usedModel: { modelName: model.modelName, modelFunction },
  ...
  // x-chat-message goes in an HTTP header; a full long prompt would blow the header
  // size limit. The real query lives in the body (and chat_token = md5(full query)),
  // so a truncated snippet here is enough for the upstream's logging/anti-bot use.
  const qSnippet = query.length > 160 ? query.slice(0, 160) : query;
  const xchat = `query:${encodeURIComponent(qSnippet)},anti_ext:${encodeURIComponent(JSON.stringify(ae))},enter_type:chat_url,re_rank:1,modelName:${model.modelName},sa:bkb`;
  return { body, xchat };
}
  ...
  try {
    while (true) {
      ...
      while ((nl = buf.indexOf('\n')) >= 0) {
        ...
        // reasoning streams as small delta fragments in reasoningContentArr (NOT
        // data.value); concatenating the per-event fragments yields the full CoT.
        else if (g.component === 'thinkingSteps' && Array.isArray(g.data?.reasoningContentArr)) {
          const t = g.data.reasoningContentArr.join('');
          if (t) yield { kind: 'reason', text: t };
        }
      }
    }
  } catch (e) {
